...
| **MIDI 裝置選擇** | 可設定偏好裝置，啟動時自動優先連接 |
| **鍵位映射查看器** | 以唯讀對話框顯示完整 MIDI-鍵盤映射表 |
| **智慧前處理** | 自動移調 → 八度摺疊 → 碰撞去重 |
| **增強熱插拔** | 每 5 秒自動偵測裝置變化，並記錄變更日誌 |
...
| **循環播放** | 可開啟循環模式自動重複播放曲目 |
| **節拍器倒數** | 可選擇性 4 拍倒數，並有視覺化指示器 |
| **進度條拖曳** | 可拖曳進度條跳轉播放位置 |
...
| **循環播放** | 按 L 鍵切換循環模式，編輯時重複播放 |
| **節拍器倒數** | 按 M 鍵切換 4 拍倒數，附視覺化指示器 |
...
| **設定對話框** | 集中管理 MIDI 裝置選擇與偏好設定 (Ctrl+,) |
...
2. **配置設定** — 按 `Ctrl+,` 開啟設定對話框，設定偏好的 MIDI 裝置
3. **查看鍵位映射** — 點選「查看映射」按鈕，檢視完整 MIDI-鍵盤配置表
4. **選擇鍵位方案** — 根據遊戲選擇對應方案（燕雲/FF14/通用）
5. **即時演奏** — 切換到遊戲視窗，開始彈奏
6. **匯入 MIDI** — 點選「曲庫」匯入 .mid 檔案，設定速度/循環後播放
7. **編輯 MIDI** — 點選「編曲器」編輯音符，按 L/M 切換循環/節拍器，匯出修改後的 MIDI
